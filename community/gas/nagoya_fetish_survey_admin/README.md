...
- **クロス集計**：「地域7ブロック×SNBC興味」の固定クロス表に加えて、選択式設問×年代・地域の
  クロス集計（詳細は下記「各設問クロス集計（選択肢×年代・地域）」を参照）。
- **自由記述一覧**：ページ下部の折りたたみ（`<details>`）にのみ表示し、トップには表示しない。

管理画面では、公開結果ページでは出さない22カテゴリの個別ランキング・地域×年代×嗜好等の
詳細クロス集計を許可している。「地域7ブロック×SNBC興味」のような1組の固定クロス表以外に、
Issue #317で選択式設問全般×年代・地域のクロス集計を追加した（下記参照）。今後さらに他の
組み合わせが必要になった場合は`buildHighlightsSection_()`・`buildCrosstabSection_()`・
`Code.gs`の集計関数を参考に追加する。

## 各設問クロス集計（選択肢 × 年代・地域）（Issue #317）

「クロス集計」セクション内、「地域7ブロック×SNBC興味」の下に、選択式設問ごとに開閉できる
アコーディオン（`<details>`）が並ぶ。各設問には次の3つのクロス表がある。

- **年代クロス**：行＝選択肢、列＝年代（`AGE_OPTIONS`の7区分）。
- **地域クロス（7地域＋海外）**：行＝選択肢、列＝`REGION_BLOCKS`（既存の全国7地域ブロック定義を
  そのまま再利用。三重県は近畿）。`REGION_BLOCKS`は国内7地域ブロックに「海外」を加えた実質8列
  なので、見出しは「7ブロック」ではなく「7地域＋海外」と表記し、列数の誤解を避けている。
- **地域詳細（東京・愛知・大阪）**：行＝選択肢、列＝東京都・愛知県・大阪府・その他の4カテゴリ固定
  （`REGION_HIGHLIGHT_PREFECTURES`）。47都道府県フルではなく、この3都府県以外（海外を含む）は
  すべて「その他」にまとめる。既存の「地域（7ブロック）」表はそのまま維持しており、この表は
  その下に追加で並ぶだけで、既存の集計・表示を変更・置換するものではない。

各セルは「件数（構成比%）」の形式で表示する。**構成比の分母は行＝選択肢の合計**（その選択肢を
選んだ人のうち何%がその年代/地域かを表す）。年代・地域側を分母にした選択率（例：30代のうち
何%がこの選択肢を選んだか）は今回のスコープ外。

### 対象設問（計21問）

`CROSSTAB_QUESTIONS`（`Code.gs`）で定義。

- `preferred_frequency`〜`gap_reasons`（#292由来・旧アンケートから継続する単一/複数選択、11問）
- `interest_categories`〜`suit_event_interest`（Issue #293以降の新設問、単一/複数選択10問。
  `primary_interest_category`＝第一嗜好も含む。「回答者を1ジャンルに固定して解釈しない」補助
  指標という位置づけ自体は変わらないが、Issueは選択式設問を原則対象としており、既存の単純集計
  （`buildCostumeSection_`）でも衣装集計の正式項目として扱っているため、クロス集計の対象にも含める）

`age`・`prefecture`自体（クロス集計の軸として使う側）と自由記述（`free_comment`）は対象外。
設問を追加・削除・変更する場合は`CROSSTAB_QUESTIONS`配列を編集するだけでよい。Dashboard側は
この配列をサーバーから受け取ったペイロードをループしてアコーディオン・表を動的生成しており、
設問ごとの手書きHTML・render呼び出しの追加は不要。

### 旧アンケート回答の除外と、上部の単純集計との分母の違い（重要）

このクロス集計セクションだけ、`isNewSurveyCompletionStage_(row.completion_stage)`
（`completion_stage`が空でないか＝Issue #293以降の新アンケート回答か）で集計対象行を絞り込んでから
`crosstabSingleVsSingle_` / `crosstabMultiVsSingle_`に渡している。`isNewSurveyCompletionStage_`は
公開プロジェクト（`nagoya_fetish_survey_webapp/Code.gs`）にある同名関数のコピーで、Apps Script
はプロジェクトをまたいだモジュール共有ができないため他の定数群と同様の対応。公開プロジェクト側で
判定ロジックを変更した場合は、このコピーも同時に更新すること。

そのため、**クロス集計の合計件数は、同じ設問の上部の単純集計（`preferred_frequency`〜`gap_reasons`
等）の合計件数より少なくなる**。単純集計は新旧アンケート回答が混在したまま集計しているのに対し、
クロス集計は現行アンケート（Issue #293以降）の回答のみを対象とするため、分母が異なる。これは
意図した挙動であり、バグではない（Dashboard画面にも同じ説明を表示している）。上部の単純集計
（`buildRegionSection_`・`buildAgeSection_`・`buildDeepDiveSection_`等）自体はこのクロス集計を
追加するにあたって一切変更していない。

### 複数選択・分岐設問の扱い

- 複数選択設問は`crosstabMultiVsSingle_`が、1行（1回答者）ごとに`seen`で選択肢を一意化してから
  加算するため、同一回答者が同じ選択肢に対して二重にカウントされることはない（Issue #317の
  レビュー指摘対応。元の実装は`splitMultiValue_`で分解した値をそのまま加算しており、同一セルに
  同じ値が複数回入っていた場合に二重カウントされ得る構造だったため、一意化のロジックを追加した）。
- `crosstabSingleVsSingle_`/`crosstabMultiVsSingle_`はどちらも、値が選択肢配列に存在しない行
  （分岐で回答していない空欄行）を自動的に除外するため、分岐設問（`barriers`等のSTEP3設問群）の
  母数はその設問に実際に到達した回答者数になる。
- 年代の「回答しない」、地域の「海外」（地域7ブロック＋海外の列では「海外」列、地域詳細では
  「その他」にバケット化）も、既存の集計仕様通りにそのままクロス集計される。
...
### 本番反映手順（Issue #317：各設問クロス集計の追加）

このIssueは管理者用GAS（本ディレクトリ）のみの変更で、公開アンケート・公開結果ページ・
回答保存処理には一切手を入れていない。`SPREADSHEET_ID`等の設定変更も不要。

1. 既存の管理者ダッシュボードのApps Scriptプロジェクトを開く。
2. `Code.gs`・`Dashboard.html`・`DashboardStyles.html`・`DashboardScript.html`の内容を、
   このディレクトリの最新版でそれぞれ上書き保存する（ファイル名は変更しない）。
3. 「デプロイ」→「デプロイを管理」→既存のWebアプリデプロイの鉛筆アイコン→
   バージョン「新しいバージョン」を選択して更新（**「アクセスできるユーザー：自分のみ」の
   設定はそのまま維持する**。新規デプロイを作るとこの設定を選び直す必要があるため、
   既存デプロイの「新しいバージョン」更新を推奨）。
4. デプロイ済みのURLを開き直し、「クロス集計」セクション内に「各設問クロス集計（選択肢×年代・
   地域）」が表示され、21件のアコーディオンが開閉できることを確認する。
...
- `test_backend.js`：`Code.gs`をNode.jsの`vm`モジュール上に読み込み、`readResponseRows_`
  ...
  含まれないことを含む）を検証する。Issue #317分として、`isNewSurveyCompletionStage_`・
  `regionHighlightForPrefecture_`・`CROSSTAB_QUESTIONS`（対象21問・重複なし・型が正しいこと。
  primary_interest_categoryを含む）・`buildCrosstabSection_`（旧アンケート除外による
  サンプルサイズ・単一選択×年代地域・複数選択の重複カウント防止（同一セル内の重複値も含む）・
  分岐設問で対象外回答が母数に混ざらないこと・年代「回答しない」/地域「海外」
  を含むケース・既存の単純集計に回帰がないこと）を検証する。
