...
**変更履歴（PR #299）**：`ENGAGEMENT_OPTIONS`・`SUIT_ENGAGEMENT_OPTIONS`から「両方」
（複数回答設問のため冗長）を削除し、公開プロジェクトと同期した。過去のresponsesには
「両方」が保存されている場合があるが書き換えていない。`tallyMulti_`は選択肢配列に無い値
（この「両方」を含む）を未知の値として静かに無視するため、旧回答の「両方」は集計には
反映されない参考値として残る。
