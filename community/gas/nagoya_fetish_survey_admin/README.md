...
- **地域クロス（7ブロック）**：行＝選択肢、列＝全国7地域ブロック（既存の`REGION_BLOCKS`をそのまま再利用。三重県は近畿）。
...
- `interest_categories`〜`suit_event_interest`のうち`primary_interest_category`を除く9問
  （Issue #293以降の新設問。第一嗜好=`primary_interest_category`は「回答者を1ジャンルに固定して
  解釈しない」補助指標のため、クロス集計の対象からは除外している）
...
- 複数選択設問は`crosstabMultiVsSingle_`が`splitMultiValue_`で分解した値ごとに1回だけカウントする
  ため、同一回答者が同じ選択肢に対して二重にカウントされることは構造的に発生しない。
...
- 年代の「回答しない」、地域の「海外」（地域7ブロックでは「海外」ブロック、地域詳細では
...
4. デプロイ済みのURLを開き直し、「クロス集計」セクション内に「各設問クロス集計（選択肢×年代・
   地域）」が表示され、20件のアコーディオンが開閉できることを確認する。
...
- `test_backend.js`：`Code.gs`をNode.jsの`vm`モジュール上に読み込み、`readResponseRows_`
  ...
  `regionHighlightForPrefecture_`・`CROSSTAB_QUESTIONS`（対象20問・重複なし・型が正しいこと）・
  `buildCrosstabSection_`（旧アンケート除外によるサンプルサイズ・単一選択×年代地域・複数選択の
  重複カウント防止・分岐設問で対象外回答が母数に混ざらないこと・年代「回答しない」/地域「海外」
