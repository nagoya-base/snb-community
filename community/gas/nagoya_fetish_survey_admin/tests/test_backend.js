...

{
  ...
  assert(questions.length === 20, 'CROSSTAB_QUESTIONS has exactly 20 target questions, got ' + questions.length);
  ...
  assert(fields.indexOf('primary_interest_category') === -1,
    'CROSSTAB_QUESTIONS excludes primary_interest_category (auxiliary indicator, per issue implementation notes)');
  questions.forEach((q) => {
...

{
  ...
  assert(barriersQ.byAge.matrix[rowIdx][colIdx] === 2,
    'multi-select × age crosstab counts one per respondent per option, no double counting, got ' + barriersQ.byAge.matrix[rowIdx][colIdx]);
}
