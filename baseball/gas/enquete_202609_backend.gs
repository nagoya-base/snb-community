 ...
 *
 * ── Issue #234 追記：22列→24列への列追加時の注意 ──
 * 既に本番 responses シートに実回答がある状態でこのコード（24列版）をデプロイする場合、
 * setupHeaderRow() は使えない（既存データがあると例外で停止する安全設計のため）。
 * その場合は、スプレッドシート上で x_follow_approval_ack / x_contact_method の2列を
 * 右端に手動追加し（既存データ行は空文字列のままでよい）、COLUMNS配列と実シートの列構成が
 * 一致していることを hasExpectedHeader_ で確認してからデプロイする。
 * 実回答が0件（新規シートなど）の場合のみ setupHeaderRow() で作り直してよい。
 *
